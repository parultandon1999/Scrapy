...
try:
    from api import broadcast_change_notification
except ImportError:
    broadcast_change_notification = None
...
dt_logger = logging.getLogger("DiffTracker")
dc_logger = logging.getLogger("DataCleaner")
ee_logger = logging.getLogger("ExtractionEngine")
...
class DiffTracker:
    ...
    def _init_diff_tables(self):
        ...
        dt_logger.info("Diff tracking tables initialized")
    ...
    def create_snapshot(self, page_id: int) -> int:
        ...
        try:
            ...
            if not page:
                dt_logger.error(f"Page {page_id} not found")
                return None
            ...
            dt_logger.info(f"Created snapshot {snapshot_id} for page {page_id} ({page['url']})")
            return snapshot_id
            
        except Exception as e:
            dt_logger.error(f"Error creating snapshot: {e}")
            conn.rollback()
            return None
        finally:
            conn.close()
    ...
    def detect_changes(self, url: str, current_page_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if not previous_snapshot:
                dt_logger.info(f"No previous snapshot found for {url}")
            ...
            dt_logger.error(f"Error detecting changes: {e}")
            conn.rollback()
            return None
        finally:
            conn.close()
...
class DataCleaner:
    ...
    @staticmethod
    def clean_url(value: str, base_url: str = None) -> Optional[str]:
        if not value:
            return None
    ...
    def clean_boolean(value: str) -> Optional[bool]:
        ...
        if value in ['true', 'yes', '1', 'on', 'available', 'in stock']:
            return True
        elif value in ['false', 'no', '0', 'off', 'unavailable', 'out of stock']:
            return False
        
        return None
    
    @staticmethod
    def clean_list(value: str, separator: str = ',') -> list:
        if not value:
            return []
    ...
    @classmethod
    def clean(cls, value: Any, data_type: str, **kwargs) -> Any:
        if value is None:
            return None
        
        cleaners = {
            'text': cls.clean_text,
            'number': cls.clean_number,
            'price': cls.clean_price,
            'integer': cls.clean_integer,
            'date': cls.clean_date,
            'email': cls.clean_email,
            'phone': cls.clean_phone,
            'url': cls.clean_url,
            'boolean': cls.clean_boolean,
            'list': cls.clean_list,
        }
        
        cleaner = cleaners.get(data_type, cls.clean_text)
        
        try:
            return cleaner(value, **kwargs) if kwargs else cleaner(value)
        except Exception as e:
            dc_logger.debug(f"Cleaning failed for type {data_type}: {e}")
            return value
...
class ExtractionEngine:
    ...
    async def extract_single_field(
        ...
        try:
            ...
            try:
                ...
                ee_logger.debug(f"Element not found: {selector}")
                return default
            ...
            cleaned_value = self.cleaner.clean(value, data_type)
            
            return cleaned_value if cleaned_value is not None else default
            
        except Exception as e:
            ee_logger.debug(f"Extraction failed for {selector}: {e}")
            return default
    ...
    async def extract_multiple_fields(
        ...
        container_selector = rule.get('selector')
        fields = rule.get('fields', {})
        limit = rule.get('limit')
        
        if not container_selector or not fields:
            return []
        ...
        try:
            ...
            try:
                ...
                ee_logger.debug(f"Container not found: {container_selector}")
                return []
            ...
            for container in containers:
                ...
                for field_name, field_rule in fields.items():
                    field_selector = field_rule.get('selector')
                    data_type = field_rule.get('type', 'text')
                    attribute = field_rule.get('attribute')
                    ...
                    try:
                        ...
                        cleaned_value = self.cleaner.clean(value, data_type)
                        item_data[field_name] = cleaned_value if cleaned_value is not None else default
                        
                    except Exception as e:
                        ee_logger.debug(f"Field extraction failed for {field_name}: {e}")
                        item_data[field_name] = default
            ...
            ee_logger.error(f"Multiple extraction failed: {e}")
            return []
    ...
    async def extract_all(
        ...
        for field_name, rule in rules.items():
            if not self.validate_rule(rule):
                ee_logger.warning(f"Skipping invalid rule: {field_name}")
                continue
            ...
            try:
                ...
                if is_multiple:
                    extracted_data[field_name] = await self.extract_multiple_fields(page, rule)
                else:
                    extracted_data[field_name] = await self.extract_single_field(page, rule)
                    
            except Exception as e:
                ee_logger.error(f"Extraction failed for {field_name}: {e}")
                extracted_data[field_name] = rule.get('default')
        
        return extracted_data
    
    async def extract_with_condition(
        self, 
        page: Page, 
        rule: Dict[str, Any]
    ) -> Optional[Any]:
        ...
        if condition:
            condition_selector = condition.get('selector')
        ...
        return await self.extract_single_field(page, rule)
    ...
    async def extract_with_fallback(
        ...
        for selector in selectors:
            try:
                ...
                if element:
                    value = await element.inner_text()
                    return self.cleaner.clean(value, data_type)
            except:
                continue
        
        return None
    
    async def extract_with_wait(
        self, 
        page: Page, 
        rule: Dict[str, Any]
    ) -> Optional[Any]:
        ...
        selector = rule.get('selector')
        wait_for = rule.get('wait_for')
        
        if wait_for:
            if wait_for.get('type') == selector:
                await page.wait_for_selector(wait_for.get('selector'), timeout=10000)
            elif wait_for.get('type') == 'function':
                await page.wait_for_function(wait_for.get('function'), timeout=10000)
    ...
    def validate_rule(self, rule: Dict[str, Any]) -> bool:
        ...
        if not rule.get('selector'):
            ee_logger.error("Rule missing 'selector' field")
            return False
        
        valid_types = ['text', 'number', 'price', 'integer', 'date', 
                       'email', 'phone', 'url', 'boolean', 'list']
        
        rule_type = rule.get('type', 'text')
        if rule_type not in valid_types:
            ee_logger.error(f"Invalid type: {rule_type}")
            return False
        
        if rule.get('multiple') and not rule.get('fields'):
            ee_logger.error("Multiple extraction requires 'fields'")
            return False
...
class Scraper:
    def __init__(
        ...
        smart_scroll_iterations=None,
        max_page_retries=None,
        max_download_retries=None,
    ):
        ...
        self.max_page_retries = max_page_retries if max_page_retries is not None else config.SCRAPER['max_page_retries']
        self.max_download_retries = max_download_retries if max_download_retries is not None else config.FILE_DOWNLOAD['max_download_retries']
        ...
        self.smart_scroll_iterations = smart_scroll_iterations if smart_scroll_iterations is not None else config.SCRAPER['smart_scroll_iterations']
    ...
    def _load_proxies(self):
        ...
        if not self.proxy_list:
            self.logger.warning(f"Proxy List '{self.proxy_list}' not found. Running without proxies.")
            return []
        
        proxies = [p for p in self.proxy_list if p]
        self.logger.info(f"Loaded {len(proxies)} proxies from {self.proxy_list}")
        return proxies
    ...
    def set_extraction_rules(self, rules: dict):
        ...
        self.logger.info(f"Loaded {len(rules)} extraction rules")
    ...
    async def detect_captcha(self, page):
        ...
        try:
            for selector in config.CAPTCHA['detection_selectors']:
                element = await page.query_selector(selector)
                if element:
                    is_visible = await element.is_visible()
                    if is_visible:
                        self.logger.warning(f"CAPTCHA DETECTED using selector: {selector}")
                        return True
            return False
        except Exception as e:
    ...
    async def wait_for_captcha_solution(self, page):
        if not config.CAPTCHA['manual_solving']:
            return False
        
        await self.pause_all_workers()
        
        self.logger.warning("CAPTCHA DETECTED - MANUAL SOLVING REQUIRED")
        self.logger.warning(f"URL: {page.url}")
        self.logger.warning(f"Waiting {config.CAPTCHA['wait_timeout']} seconds for you to solve it...")
        self.logger.warning("Please solve the captcha in the browser window")
        
        await self.alert_captcha()
        await self.notify_captcha(page.url)
        
        timeout = config.CAPTCHA['wait_timeout']
        ...
        while True:
            ...
            if elapsed > timeout:
                self.logger.error("Captcha solving timeout! Moving to next page...")
                await self.resume_all_workers()
                return False
            
            captcha_present = await self.detect_captcha(page)
            
            if not captcha_present:
                self.logger.info("Captcha solved successfully! Continuing...")
    ...
    async def pause_all_workers(self):
        if not config.CAPTCHA.get('pause_workers', False):
            return
        
        self.is_paused = True
        self.logger.warning("ALL WORKERS PAUSED DUE TO CAPTCHA")
    ...
    async def resume_all_workers(self):
        ...
        self.is_paused = False
        self.logger.info("ALL WORKERS RESUMED - CAPTCHA SOLVED")


    async def alert_captcha(self):
        if not config.CAPTCHA.get('sound_alert', False):
            return
        
        try:
            frequency = config.CAPTCHA.get('sound_frequency', 1000)
            duration = config.CAPTCHA.get('sound_duration', 500)
            repeat = config.CAPTCHA.get('sound_repeat', 3)
            
            self.logger.info(f"Playing sound alert ({repeat} beeps)")
    ...
    async def notify_captcha(self, url):
        if not config.CAPTCHA.get('desktop_notification', False):
            return
        
        if not PLYER_AVAILABLE:
            self.logger.debug("Desktop notifications disabled (plyer not installed)")
            return
        ...
        try:
            ...
            notification.notify(
                ...
                message=f"Please solve captcha at:\n{display_url}",
        ...
        try:
            ...
            if self.success_indicator:
                ...
                if current_url != self.login_url:
                    ...
                    error_selectors = config.AUTH['error_selectors']
    ...
    def _init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS custom_extracted_data (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                page_id INTEGER,
                field_name TEXT,
                field_value TEXT,
    ...
    async def _download_file(self, file_url, save_path, session):
        result = {
            'success': False,
            'file_size': 0,
            'error': None,
        ...
        for attempt in range(self.max_download_retries):
            try:
                async with session.get(file_url, timeout=aiohttp.ClientTimeout(config.FILE_DOWNLOAD['download_timeout'])) as response:
                    if response.status == 200:
                        ...
                        with open(save_path, 'wb') as f:
                            total_downloaded = 0
                            async for chunk in response.content.iter_chunked(config.FILE_DOWNLOAD['chunk_size']):
                                f.write(chunk)
    ...
    async def smart_scroll(self, page):
        try:
            ...
            for i in range(self.smart_scroll_iterations):
    ...
    async def extract_and_save_data(self, page, depth, proxy_used, fingerprint):
        ...
        if self.extraction_rules:
            self.logger.info("Applying custom extraction rules...")
            custom_data = await self.extraction_engine.extract_all(page, self.extraction_rules)
            self.logger.info(f"Extracted {len(custom_data)} custom fields")
        ...
        if custom_data:
            for field_name, field_value in custom_data.items():
                ...
                cursor.execute('''
                    INSERT INTO custom_extracted_data (page_id, field_name, field_value, field_type)
                    VALUES (?, ?, ?, ?)
                ''', (page_id, field_name, str(field_value), field_type))
                    ...
                    if change_result and change_result.get('changes_detected'):
                        ...
                        try:
                            for change in change_result.get('changes', []):
                                asyncio.create_task(broadcast_change_notification(url, change))
                        except Exception as broadcast_error:
                            self.logger.error(f"Failed to broadcast change notification: {broadcast_error}")
                except Exception as e:
    ...
    async def create_context(self, browser, proxy=None, storage_state=None):
        if config.FEATURES['use_fingerprinting']:
            fingerprint = self._generate_fingerprint()
    ...
    async def process_page(self, browser, url, depth):
        ...
        while retry_count < self.max_page_retries:
            try:
                ...
                if captcha_detected:
                    captcha_solved = await self.wait_for_captcha_solution(page)
                    if not captcha_solved:
                        self.logger.error(f"Failed to solve captcha for {url}")
                        return
                ...
                if proxy and any(keyword in error_msg.lower() for keyword in 
                    ...
                    proxy = await self._get_next_proxy()
                    self.logger.warning(f"Retrying with different proxy ({retry_count}/{self.max_page_retries})")
                else:
                    self.logger.error(f"Failed {url}: {e}")
                    if retry_count < self.max_page_retries:
                        self.logger.info(f"Retrying ({retry_count}/{self.max_page_retries})")
                        await asyncio.sleep(2)
    ...
    async def worker(self, browser, worker_id):
        while True:
            ...
            if url:
                ...
                await asyncio.sleep(0.5)
                async with self.lock:
                    if not self.queue and self.pages_scraped >= self.max_pages:
                        break
